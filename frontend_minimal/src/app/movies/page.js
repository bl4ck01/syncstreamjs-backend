...
import { getVodCategories, getVodStreamsByCategory, getStreamCountByCategory } from '../duckdb/queries';
...
export default function MoviesPage() {
  ...
  const loadStreamCounts = async () => {
    try {
      const counts = {};
      await Promise.all(
        categories.vod.map(async (category) => {
          counts[category.category_id] = await getStreamCountByCategory(category.category_id, 'vod');
        })
      );
  ...
  const loadStreams = async () => {
    ...
    setLoading(true);
    try {
      const streamsData = await getVodStreamsByCategory(selectedCategory.category_id, 20);
  ...
  if (loading && categories.vod.length === 0) {
    return (
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {!selectedCategory ? (
          /* Categories View */
          <div>
            <h2 className="text-2xl font-bold mb-6">Movie Categories</h2>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
              {categories.vod.map((category) => (
                <button
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs text-gray-400">
                      {streamCounts[category.category_id] || 0} titles
                    </span>
                  </div>
                  <h3 className="font-medium text-sm group-hover:text-green-300 transition-colors line-clamp-2">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">
                  {streamCounts[selectedCategory.category_id] || 0} titles available
                </p>
              </div>
            </div>
