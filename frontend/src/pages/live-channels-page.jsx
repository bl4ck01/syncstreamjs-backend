...
import { Tv, Wifi, WifiOff, Search, Database } from 'lucide-react';
...
export default function LivePage() {
  ...
  console.log('[LivePage] 📊 Store state:', {
    isInitialized,
    globalLoading,
    error,
  ...
  useEffect(() => {
    console.log('[LivePage] 🚀 Component mounted, calling initializeStore...');
    initializeStore();
  }, [initializeStore]);
  ...
  useEffect(() => {
    if (isInitialized && !currentPlaylist && !hasTriedLoading && !globalLoading) {
      console.log('[LivePage] 🎯 Loading default playlist...');
      setHasTriedLoading(true);
      loadDefaultPlaylist().then(result => {
        console.log('[LivePage] 📝 Default playlist result:', result);
        ...
        console.error('[LivePage] ❌ Error loading default playlist:', error);
      });
    }
  ...
  const categorizedData = useMemo(() => {
    ...
    try {
      ...
      if (playlistKey) {
        categorizedStreams = getCategorizedStreams(playlistKey, 'live');
      } else {
        categorizedStreams = currentPlaylist.categorizedStreams?.live || [];
      }

      if (!Array.isArray(categorizedStreams)) {
        console.warn('[LivePage] ⚠️ categorizedStreams is not an array:', categorizedStreams);
        return [];
      }
      ...
      console.log('[LivePage] ✅ Processed categories:', result.length);
      return result;
    } catch (error) {
      console.error('[LivePage] ❌ Error processing categorized data:', error);
      return [];
    }
    ...
    categorizedData.slice(0, 3).forEach(category => {
      if (category.items && category.items.length > 0) {
        featuredItems.push({
          ...
          description: `Live ${category.name} channel`
        });
      }
    });
          ...
          {noPlaylistMessage && !currentPlaylist ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No live channels found</h3>
